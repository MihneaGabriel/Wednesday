...
public:
    ...
    friend class DISH;
    friend double Total( DISH cost );
};
// Metode Event
void EVENT::Transport( int nr_pers )
{
...
void EVENT::Costume( )
{
    ...
    int j = 0;
    double totalcost = 0;
    string masc[5] = {"Vampir","Varcolac","Sirena","Clarvazator","Om"};
...
void EVENT::Food( int nr_pers )
{
    string types[3] = {"Apta sa manance orice","Vegetariana","Flexitariana"};
    srand(time(0));

    for( int i = 0 ; i < nr_pers ; i++ )
        food[i] = types[rand() % 3 ];
}
void EVENT::Room( int nr_pers )
{
    ifstream fin;
    ofstream fout;
...
public:
    ...
    DISH( EVENT obj , int nr_pers );   // Generate types of food
    ~DISH( )
    {
        cout<<"Meniul a fost pregatit cu Succes!"<<endl;
    }
...
//Constructori DISH
DISH::DISH( EVENT obj , int nr_pers )
{
    // Consideram ca supa nu contine carne
    ofstream fout;
    string supa[7] = {"Supa de legume", "Ciorba de pui", "Ciorba de vacuta", "Supa crema de ciuperci", "Supa crema de linte", "Ciorba de burta", "Ciorba de peste" };
    string fprincip[6] = {"Paste cu legume", "Curry cu pui", "Paste cu ton", "legume gratinate", "Noodels vegan", "Salata de ton" };
    string desert[5] = {"Tiramisu", "Cheesecake", "Pavlova", "Bannana bread", "Lava cake" };
    ...
    try
    {
        ...
        {
            if( obj.food[i] == "Apta sa manance orice")
            {
                fout<<supa[rand()%7]<<","
                    <<fprincip[rand()%6]<<","
                    <<desert[rand()%5]<<",,";
            ...
            else if( obj.food[i] == "Vegetariana" )
            {
                string aux = "";
                while( aux == "" ) // always find a var
                    aux = Checker( supa[rand()%7], "Supa" );
                fout<<aux<<",";

                aux = "";
                while( aux == "" )
                    aux = Checker( fprincip[rand()%6], "legume");
                fout<<aux<<","
                    <<desert[rand()%5]<<",,"; // All vegetarians dont have restriction to serve desert

                totalcost[1] = totalcost[1] + 33;
            }
            else if( obj.food[i] == "Flexitariana")
            {
                string aux = "";
                while( aux == "" ) // always find a var
                    aux = Checker( supa[rand()%7], "peste" );
                fout<<aux<<",";

                aux = "";
                while( aux == "" )
                    aux = Checker( fprincip[rand()%6], "ton");
                fout<<aux<<","
                    <<desert[rand()%5]<<",,";
...
// Metode DISH
string DISH::Checker( string aux, string key )
{
...
template<class T>
T Figure() // Generate random names (RANDOM)
{
...
int mainRO()
{
    ...
    nr_pers = Figure<int>();
    cout<<"Introduceti un nr de zile: ";
    cin>>nr_days;
    ...
    cout<<"Costul filmarilor este de "<<(int)Total(d)<<" $"<<endl<<endl;
